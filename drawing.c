...
static void pfield_do_linetoscr (int start, int stop)
{
	if (currprefs.chipset_mask & CSMASK_AGA)
	{
    		switch(res_shift)
		{
			case 0 : src_pixel = linetoscr_16_aga (src_pixel, start, stop); break;
			case 1 : src_pixel = linetoscr_16_stretch1_aga (src_pixel, start, stop); break;
			case -1: src_pixel = linetoscr_16_shrink1_aga (src_pixel, start, stop); break;
			default: abort ();
		}
	}
	else
	{
    		switch(res_shift)
		{
			case 0 : src_pixel = linetoscr_16 (src_pixel, start, stop); break;
			case 1 : src_pixel = linetoscr_16_stretch1 (src_pixel, start, stop); break;
			case -1: src_pixel = linetoscr_16_shrink1 (src_pixel, start, stop); break;
			default: abort ();
		}
	}
}
...
{
uae_u16 *b = (uae_u16 *)xlinebuffer;
xcolnr col = colors_for_drawing.acolors[0];
int i;

	for (i = start; i < stop; i++) b[i] = col;
}
...
STATIC_INLINE void pfield_draw_line (int lineno, int gfx_ypos, int follow_ypos)
{
	...
	if (dh == dh_emerg)
	    memcpy (row_map[gfx_ypos], xlinebuffer + linetoscr_x_adjust_bytes, 720);

	do_flush_line (gfx_ypos);
	if (do_double) {
	    if (dh == dh_emerg)
		memcpy (row_map[follow_ypos], xlinebuffer + linetoscr_x_adjust_bytes, 720);
	    else if (dh == dh_buf)
		memcpy (row_map[follow_ypos], row_map[gfx_ypos], 720);
...
extern int g_draw_status; //UAE-X
...
int i;
	...
	//UAE-X
	if(g_draw_status && where >= gfxvidinfo.height - TD_TOTAL_HEIGHT) break;
	if(!g_draw_status && where >= gfxvidinfo.height) break; 
